...
def admin_view_agent_view(request):
    agents= AMODEL.Agent.objects.all()
    return render(request,'insurance/admin_view_agent.html',{'agents':agents})

def admin_agent_view(request):
    return render(request,'insurance/admin_agent.html')
...
def admin_view_customer_view(request):
    ...
    customers_agent_code=list(CMODEL.Customer.objects.values_list('agent_code',flat=True))
    # AMODEL.Agent.objects.values('agent_code','agent_name')
...
def delete_agent_view(request,pk):
    agent=AMODEL.Agent.objects.get(id=pk)
